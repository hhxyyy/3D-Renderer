...
int main(int argc, char** argv)
{
    ...
    if (!config.cliRenderingEnabled) {
        ...
        int bvhDebugLevel = 0;
        int bvhDebugLeaf = 0;
        int motionSampleNumber = 0;
        float motionDistanceX = 0.3f;
        float motionDistanceY = 0;
        float motionDistanceZ = 0;
        float dofFocal = 3.0f;
        float aperture = 0.1f;
        int bloomFilterSize = 0;
        int binsNumber = 2;
        bool debugBloomFilterSize { false };
        bool debugMotionSamples { false };
        bool debugMotionDistance { false };
        bool debugDof { false };
        ...
        while (!window.shouldClose()) {
            ...
            ImGui::Begin("Final Project");
            {
                constexpr std::array items {
                    ...
                    "Dragon",
                    /* "AABBs",*/ "Spheres", /*"Mixed",*/
                    "Custom", "Debug Textures", "Debug Bilinear", "Debug MipMap"
                };
            ...
            if (ImGui::CollapsingHeader("Features", ImGuiTreeNodeFlags_DefaultOpen)) {
                ...
                ImGui::Checkbox("BVH debug", &config.features.enableBVHDebug);
            ...
            if (ImGui::CollapsingHeader("Extra Features")) {
                ...
                if (config.features.extra.enableBvhSahBinning)
                    ImGui::SliderInt("Bins", &binsNumber, 2, 10);
                ...
                ImGui::Checkbox("Multiple Rays", &config.features.extra.enableMultipleRaysPerPixel);
                ImGui::Checkbox("Multiple Rays Debug", &config.features.extra.enableDebugMultipleRays);
            }
            ImGui::Separator();
            ...
            if (viewMode == ViewMode::Rasterization) {
                ...
                ImGui::Checkbox("Control Motion Samples", &debugMotionSamples);
                if (debugMotionSamples)
                    ImGui::SliderInt("Motion Blur Samples", &motionSampleNumber, 0, 150);
                ImGui::Checkbox("Control Motion Distance",  &debugMotionDistance);
                if (debugMotionDistance) {
                    ImGui::SliderFloat("Motion Distance X", &motionDistanceX, 0.0f, 5.0f);
                    ImGui::SliderFloat("Motion Distance Y", &motionDistanceY, 0.0f, 5.0f);
                    ImGui::SliderFloat("Motion Distance Z", &motionDistanceZ, 0.0f, 5.0f);
                }
                ImGui::Checkbox("Control Bloom Effect", &debugBloomFilterSize);
                if (debugBloomFilterSize)
                    ImGui::SliderInt("Filter Size", &bloomFilterSize, 0, 12);

                ImGui::Checkbox("Debug DOF", &debugDof);
                if (debugDof) {
                    ImGui::SliderFloat("Focal", &dofFocal, 0.0f, 5.0f);
                    ImGui::SliderFloat("Aperture", &aperture, 0.0f, 1.0f);
                }
            }
            if (viewMode == ViewMode::RayTracing) {
                ImGui::Checkbox("Control Motion Samples", &debugMotionSamples);
                if (debugMotionSamples)
                    ImGui::SliderInt("Motion Blur Samples", &motionSampleNumber, 0, 150);
                ImGui::Checkbox("Control Motion Distance", &debugMotionDistance);
                if (debugMotionDistance) {
                    ImGui::SliderFloat("Motion Distance X", &motionDistanceX, 0.0f, 0.5f);
                    ImGui::SliderFloat("Motion Distance Y", &motionDistanceY, 0.0f, 0.5f);
                    ImGui::SliderFloat("Motion Distance Z", &motionDistanceZ, 0.0f, 0.5f);
                }
                ImGui::Checkbox("Control Bloom Effect", &debugBloomFilterSize);
                if (debugBloomFilterSize)
                    ImGui::SliderInt("Filter Size", &bloomFilterSize, 0, 12);
            ...
            case ViewMode::Rasterization: {
                ...
                if (debugBVHLevel || debugBVHLeaf) {
                    ...
                    enableDebugDraw = true;
                    if (debugBVHLevel)
                        bvh.debugDrawLevel(bvhDebugLevel, config.features);
                    if (debugBVHLeaf)
                ...
                if (debugMotionSamples) {
                    setDivision(motionSampleNumber);
                }
                if (debugMotionDistance) {
                    enableDebugDraw = true;
                    glm::vec3 motionDistance { motionDistanceX, motionDistanceY, motionDistanceZ };
                    setMotionDistance(motionDistance);
                    debugMotion(motionDistance);
                    enableDebugDraw = false;
                }
                if (debugBloomFilterSize) {
                    setBloomFilterSize(bloomFilterSize);
                }
                if (debugDof) {
                    enableDebugDraw = true;
                    debugDOF(screen, camera);
                    enableDebugDraw = false;
                }

                if (config.features.extra.enableBvhSahBinning) {
                    setBinsNumber(binsNumber);
                }

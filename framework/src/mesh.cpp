        ...
        for (size_t endTriangle = 0; endTriangle < shape.mesh.indices.size() / 3; ++endTriangle) {
            ...
            if (materialID == -1) {
                ...
                if (!objMaterial.diffuse_texname.empty()) {
                    ...
                    // Generate mipmaps levels for a given mesh
                    generateMipMaps(mesh, *mesh.material.kdTexture);
                }
...
// Generates next level (4 times smaller than previous)
Image generateNextLevel(const Image& image)
{
    std::vector<glm::vec3> pixels;
    int i = 0;
    while (i < image.pixels.size()) {
        glm::vec3 a = image.pixels[i];
        glm::vec3 b = image.pixels[i + 1];
        glm::vec3 c = image.pixels[i + image.width];
        glm::vec3 d = image.pixels[i + image.width + 1];
        glm::vec3 pixel = 0.25f * a + 0.25f * b + 0.25f * c + 0.25f * d;
    ...
    nextLevel.width = image.width / 2;
    nextLevel.height = image.height / 2;
    nextLevel.pixels = pixels;
    return nextLevel;
}

// Generate all levels of MipMap for a given image
void generateMipMaps(Mesh& mesh, const Image& image) {
    if (image.width != image.height || ((image.width - 1) & image.width) != 0)
        return;
    mesh.mipmaps.push_back(image);
    int n = image.width;
    while (n > 1) {
        mesh.mipmaps.push_back(generateNextLevel(mesh.mipmaps[mesh.mipmaps.size() - 1]));
        n = n / 2;
    }
}